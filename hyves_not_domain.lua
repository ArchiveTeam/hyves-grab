...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if string.match(url, "hyves.nl"..hyves_username.."[%w_-]+/?$") then
    if not html then
      html = read_file(file)
    end
    io.stdout:write("\n  Trying to paginate "..string.match(url, "hyves.nl"..hyves_username.."([%w_-]+)/?$").."\n")
    io.stdout:flush()
  ...
  if string.match(url, "hyves.nl"..hyves_username.."fotos/") or
  string.match(url, "hyves.nl"..hyves_username.."photos/") or
  string.match(url, "hyves.nl"..hyves_username.."album/") then
    if not html then
      html = read_file(file)
    end
  ...
  string.match(url, "module=PhotoBrowser&action=postGetSocialPage") then
    ...
    for requisite_url in string.gmatch(html, "[cf]=['\"](https?://www%.hyves%.nl"..hyves_username.."[^'\"]+)['\"]") do
  ...
  photo_urls[url] then
    ...
    local photo_meta_url = "http://www.hyves.nl/?module=PhotoBrowser&action=postGetSocialPage"
